...
log_path   = os.path.join(OUTPUT_DIR, "train_integrated_gradients_log.txt")
...
for plot_idx, (dict_idx, data) in enumerate(gradients_dict.items()):
    ...
    plt.tight_layout()
    filename = os.path.join(OUTPUT_DIR, f'train_integrated_gradients_{plot_idx:02d}_{CLASS_NAMES[data["pred_class"]]}.png')
...
plt.tight_layout()
plt.savefig('train_integrated_gradients_per_classe.png', dpi=150, bbox_inches='tight')
plt.close()
print("Salvato: train_integrated_gradients_per_classe.png\n")
...
GRAY_VALUE = 0.5 # Le immagini denormalizzate sono in range [0, 1]
...
for alpha in ALPHAS:
    ...
    for dict_idx, data in gradients_dict.items():
        true_label = data['true_label']
        class_name = CLASS_NAMES[true_label]
        
        # 1. Recupero immagine base
        # L'immagine salvata è normalizzata, la denormalizzo in [0,1]
        img_base = denormalize(data['image']).permute(1, 2, 0).numpy().copy()
        ...
        # 2. Calcolo mappa di importanza (valore assoluto, media sui canali)
        ig_tensor = data['integrated_grads']
        ig_abs = torch.abs(ig_tensor).mean(dim=0).numpy() # Shape: (H, W)
        flat_ig = ig_abs.flatten()
        
        # Mascheramento XAI (Seleziona i top-k pixel)
        top_k_indices_xai = np.argsort(flat_ig)[-k:]
        ...
        # Mascheramento RANDOM (Seleziona k pixel a caso)
