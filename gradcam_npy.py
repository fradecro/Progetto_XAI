...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("SALVATAGGIO MAPPE GRADCAM COME TENSORI NPY (PER ROAR)")
    print("=" * 80)

    TRAIN_DIR = "seg_train/seg_train" 
    TENSORS_TRAIN_DIR = "gradcam_tensors/train"
    TENSORS_TEST_DIR  = "gradcam_tensors/test"
    IMAGES_TRAIN_DIR  = "gradcam_images/train"
    IMAGES_TEST_DIR   = "gradcam_images/test"

    for d in [TENSORS_TRAIN_DIR, TENSORS_TEST_DIR, IMAGES_TRAIN_DIR, IMAGES_TEST_DIR]:
        os.makedirs(d, exist_ok=True)

    def elabora_e_salva_dataset_gradcam(dataset_dir, out_tensors_dir, out_images_dir):
        full_ds = datasets.ImageFolder(root=dataset_dir, transform=transform)
        dataloader_full = DataLoader(full_ds, batch_size=1, shuffle=False)
        print(f"\nElaborazione di {len(full_ds)} immagini da {dataset_dir}...")
        
        with GradCAM(model=model, target_layers=[target_layer]) as cam:
            for idx, (img_tensor, label_idx) in enumerate(dataloader_full):
                img_path, _ = full_ds.samples[idx]
                ...
                # Salva tensore
                np.save(save_npy_path, grayscale_cam)
                ...
                rgb_img = cv2.resize(np.array(raw_image, dtype=np.float32) / 255.0, (W, H))
                cam_image = show_cam_on_image(rgb_img, grayscale_cam, use_rgb=True)
                cv2.imwrite(save_img_path, cv2.cvtColor(cam_image, cv2.COLOR_RGB2BGR))
                
                if (idx + 1) % 500 == 0:
                    print(f"Salvate {idx + 1}/{len(full_ds)} mappe e immagini...")

    print("INIZIO ESTRAZIONE MAPPE TEST SET")
    elabora_e_salva_dataset_gradcam(dataset_path, TENSORS_TEST_DIR, IMAGES_TEST_DIR)

    print("\nINIZIO ESTRAZIONE MAPPE TRAIN SET")
    elabora_e_salva_dataset_gradcam(TRAIN_DIR, TENSORS_TRAIN_DIR, IMAGES_TRAIN_DIR)

    print("\nTutte le mappe GradCAM sono state salvate con successo!")