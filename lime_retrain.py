...
log_path   = os.path.join(OUTPUT_DIR, "train_lime_log.txt")
...
for idx, (_, label) in enumerate(test_dataset):
    if class_counts[label] < 85:
        selected_indices.append(idx)
        class_counts[label] += 1
    if len(selected_indices) >= NUM_CLASS * 85:
        break
...
for plot_idx, (dict_idx, data) in enumerate(lime_results.items()):
    ...
    fname = os.path.join(OUTPUT_DIR, f'train_lime_analisi_{plot_idx:02d}_{CLASS_NAMES[pred_class]}.png')
...
plt.tight_layout()
plt.savefig('train_lime_media_per_classe.png', dpi=150, bbox_inches='tight')
plt.close()
print("Salvato: train_lime_media_per_classe.png\n")
...
# ==========================================
print("\n" + "=" * 80)
print("AVVIO FAITHFULL EVALUATION (ROAR) - MASCHERAMENTO XAI & RANDOM")
print("=" * 80)

ROAR_BASE_DIR = os.path.join(OUTPUT_DIR, "roar_datasets")
ALPHAS = [0.1, 0.3, 0.5, 0.7, 0.9]
GRAY_VALUE = 0.5 # Le immagini di LIME raw sono state mappate su [0, 1]

print(f"Creazione dataset mascherati in: {ROAR_BASE_DIR}")
print(f"Soglie alpha: {[int(a*100) for a in ALPHAS]}%")
print(f"Colore di mascheramento (grigio fisso): {GRAY_VALUE} (in scala 0-1)\n")

for alpha in ALPHAS:
    alpha_pct = int(alpha * 100)
    
    # Creazione cartelle per XAI e RANDOM (sotto la directory di LIME)
    xai_dir = os.path.join(ROAR_BASE_DIR, "xai", f"alpha_{alpha_pct}", "train")
    rnd_dir = os.path.join(ROAR_BASE_DIR, "random", f"alpha_{alpha_pct}", "train")
    
    for c_name in CLASS_NAMES:
        os.makedirs(os.path.join(xai_dir, c_name), exist_ok=True)
        os.makedirs(os.path.join(rnd_dir, c_name), exist_ok=True)
        
    print(f" -> Elaborazione soglia Alpha = {alpha_pct}%")
    
    for dict_idx, data in lime_results.items():
        true_label = data['true_label']
        class_name = CLASS_NAMES[true_label]
        
        # 1. Recupero immagine base in [0, 1]
        img_base = data['img_np'].copy()
        ...
        # Costruisco una mappa continua dei pesi assoluti di LIME a livello di pixel
        weight_map = np.zeros(segments.shape, dtype=float)
        local_exp = expl.local_exp[pred_class]
        
        for seg_id, weight in local_exp:
            # Assegno il peso assoluto del superpixel a tutti i pixel che vi appartengono
            weight_map[segments == seg_id] = np.abs(weight)
            
        flat_lime = weight_map.flatten()
        
        # 2. Mascheramento XAI (Seleziona i top-k pixel per importanza, dedotti dai superpixel)
        top_k_indices_xai = np.argsort(flat_lime)[-k:]
        mask_xai = np.zeros(num_pixels, dtype=bool)
        mask_xai[top_k_indices_xai] = True
        mask_xai_2d = mask_xai.reshape((H, W))
        
        img_xai = img_base.copy()
        img_xai[mask_xai_2d] = GRAY_VALUE
        
        # 3. Mascheramento RANDOM (Seleziona k pixel puramente a caso)
        ...
        img_rnd = img_base.copy()
        img_rnd[mask_rnd_2d] = GRAY_VALUE
        
        # 4. Salvataggio delle immagini su disco
        img_xai_255 = (img_xai * 255).astype(np.uint8)
        img_rnd_255 = (img_rnd * 255).astype(np.uint8)
        
        im_xai = Image.fromarray(img_xai_255)
        im_rnd = Image.fromarray(img_rnd_255)
        
        im_xai.save(os.path.join(xai_dir, class_name, f"img_{dict_idx:04d}.png"))
        im_rnd.save(os.path.join(rnd_dir, class_name, f"img_{dict_idx:04d}.png"))

print("Generazione dataset mascherati completata.\n")
