...
# Salvataggio tensori .npy
print("\n" + "=" * 80)
print("SALVATAGGIO MAPPE INTEGRATED GRADIENTS COME TENSORI NPY E PLOT PNG (PER ROAR)")
print("=" * 80)

TRAIN_DIR = "seg_train/seg_train"
TENSORS_TRAIN_DIR = "ig_tensors/train"
TENSORS_TEST_DIR  = "ig_tensors/test"
IMAGES_TRAIN_DIR  = "ig_images/train"
IMAGES_TEST_DIR   = "ig_images/test"

for d in [TENSORS_TRAIN_DIR, TENSORS_TEST_DIR, IMAGES_TRAIN_DIR, IMAGES_TEST_DIR]:
    os.makedirs(d, exist_ok=True)

def elabora_e_salva_dataset_ig(dataset_dir, out_tensors_dir, out_images_dir):
    ...
    for idx in range(len(full_dataset)):
        ...
        with torch.no_grad():
            ...
            pred_class = int(np.argmax(torch.softmax(output, dim=1)[0].cpu().numpy()))
        
        # Calcolo Integrated Gradients (usando la funzione locale, steps=30 per equilibrio velocità/qualità)
        ig = integrated_gradients(model, img_batch, pred_class, steps=30)
        
        # Valore assoluto e media sui canali per creare una mappa di salienza 2D (H, W)
        ig_2d = torch.abs(ig).squeeze(0).mean(dim=0).cpu().numpy()
        
        np.save(save_npy_path, ig_2d)
        
        # Salvataggio immagine PNG
        plt.figure(figsize=(6, 6))
        img_denorm = denormalize(img_tensor).permute(1, 2, 0).numpy()
        plt.imshow(img_denorm, alpha=0.6)
        plt.imshow(ig_2d, cmap='hot', alpha=0.4)
        plt.axis('off')
        plt.savefig(save_img_path, dpi=100, bbox_inches='tight')
        plt.close()
        
        if (idx + 1) % 50 == 0:
            print(f"Salvate {idx + 1}/{len(full_dataset)} mappe e immagini...")

print("INIZIO ESTRAZIONE MAPPE TEST SET")
elabora_e_salva_dataset_ig(TEST_DIR, TENSORS_TEST_DIR, IMAGES_TEST_DIR)

print("\nINIZIO ESTRAZIONE MAPPE TRAIN SET")
elabora_e_salva_dataset_ig(TRAIN_DIR, TENSORS_TRAIN_DIR, IMAGES_TRAIN_DIR)

print("\nTutte le mappe Integrated Gradients sono state salvate con successo!")
