...
TENSORS_TRAIN_DIR = "lime_tensors/train"
TENSORS_TEST_DIR  = "lime_tensors/test"
IMAGES_TRAIN_DIR  = "lime_images/train"
IMAGES_TEST_DIR   = "lime_images/test"
TRAIN_DIR = "seg_train/seg_train"

for d in [TENSORS_TRAIN_DIR, TENSORS_TEST_DIR, IMAGES_TRAIN_DIR, IMAGES_TEST_DIR]:
    os.makedirs(d, exist_ok=True)

def elabora_e_salva_dataset_lime(dataset_dir, out_tensors_dir, out_images_dir):
    full_dataset_raw = datasets.ImageFolder(dataset_dir, transform=raw_transforms)
    
    for idx in range(len(full_dataset_raw)):
        img_raw, _ = full_dataset_raw[idx]
        ...
        with torch.no_grad():
            norm_img = normalize(img_raw).unsqueeze(0).to(DEVICE)
        ...
        lime_2d = np.abs(weight_map)
        np.save(save_npy_path, lime_2d)
...
elabora_e_salva_dataset_lime(TEST_DIR, TENSORS_TEST_DIR, IMAGES_TEST_DIR)
elabora_e_salva_dataset_lime(TRAIN_DIR, TENSORS_TRAIN_DIR, IMAGES_TRAIN_DIR)
