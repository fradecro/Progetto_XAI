...
log_path   = os.path.join(OUTPUT_DIR, "train_shap_log.txt")
...
fname = os.path.join(OUTPUT_DIR, 'train_shap_image_plot.png')
...
GRAY_VALUE = 0.5 # Le immagini test_images_np sono in range [0, 1]
...
for alpha in ALPHAS:
    ...
    # Creazione cartelle per XAI e RANDOM
    ...
    for img_idx in range(len(test_images)):
        ...
        # 1. Recupero immagine e mappa SHAP
        # L'immagine base (H, W, C) con valori in [0, 1]
        img_base = test_images_np[img_idx].copy() 
        ...
        # Calcolo della mappa 2D media per l'immagine corrente
        shap_img = shap_values.values[img_idx]        # (H, W, C, k_outputs)
        shap_2d  = np.abs(shap_img).mean(axis=(2, 3)) # (H, W)
        flat_shap = shap_2d.flatten()
        
        # 2. Mascheramento XAI (Seleziona i top-k pixel per importanza)
        top_k_indices_xai = np.argsort(flat_shap)[-k:]
        ...
        # 3. Mascheramento RANDOM (Seleziona k pixel a caso)
        ...
        # Riporto nel range [0, 255] per salvare come PNG
        img_xai_255 = (img_xai * 255).astype(np.uint8)
        img_rnd_255 = (img_rnd * 255).astype(np.uint8)
        
        # Pil Image vuole (W, H, C) e format uint8
        im_xai = Image.fromarray(img_xai_255)
        im_rnd = Image.fromarray(img_rnd_255)
        
        im_xai.save(os.path.join(xai_dir, class_name, f"img_{img_idx:04d}.png"))
        im_rnd.save(os.path.join(rnd_dir, class_name, f"img_{img_idx:04d}.png"))
